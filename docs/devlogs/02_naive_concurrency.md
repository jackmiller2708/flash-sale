...
## Context
Phase 1 involved implementing the core order flow. The "naive" part refers to using a single row for inventory and locking it during the entire transaction. During this phase, we also aligned the project's architectural patterns to ensure consistency as we scale.

## Lessons Learned

### Terminology Alignment: Purchase vs. Order
We decided to harmonize the terminology between the HTTP layer and the domain logic. 
...
To decouple our domain objects from the database schema, we adopted a standard "Record" pattern across all repositories.
- **Pattern**: For each repository (e.g., `Order`, `FlashSale`), we introduced a corresponding `*Record` struct (e.g., `OrderRecord`) that implements `sqlx::FromRow`.
...
- **Analysis**:
    ...
    - **Correction**: Initial tests hit foreign key violations because of random `user_id` generation. The load test was updated to fetch real users from the `/users` endpoint in the `setup` phase, ensuring valid references.
